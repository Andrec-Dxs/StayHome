...
namespace CoronaStatusUpdateIOSServices
{
    /// <summary>
    ...
    public class CustomNavigationService : IMyNavigationService
    {
        /// <summary>
        ...
        protected readonly Dictionary<string, TypeActionOrKey> _pagesByKey = new Dictionary<string, TypeActionOrKey>();

        protected readonly Dictionary<WeakReference, object> _parametersByController =
        ...
        public void NavigateTo(string pageKey, object parameter)
        {
            lock (_pagesByKey)
            {
                ...
                if (!string.IsNullOrEmpty(item.StoryboardControllerKey))
                {
                    ...
                    if (parameter != null)
                    {
                        var casted = controller as StayHome.IOS.Services.ControllerBase;
                        if (casted != null)
                        {
        ...
        protected void PushViewWithHistory(UIViewController controller, bool animated)
        {
            if (NavigationController.ViewControllers.Length > 0)
            {
                if (NavigationController.TopViewController is HistoryBaseViewController && (NavigationController.TopViewController as HistoryBaseViewController).NoHistory)
                    NavigationController.PopViewController(false);
            }
            NavigationController.PushViewController(controller, true);
        }

        protected UIViewController MakeController(Type controllerType, object parameter)
        {
        ...
        protected class TypeActionOrKey
        {
